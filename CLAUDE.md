...
- **PBS batch** — the `qsub` job scripts (`#PBS -l nodes=...`) live in **`jobs/`**. They `cd` into the hard-coded cluster path and call a specific Python venv (e.g. `.venv_jax/bin/python` for JAX-CPU). Submit from the repo root with the `jobs/` prefix, e.g. `for i in 0..9; do qsub -v FOLD=$i jobs/loo_glv_micom_job.sh; done`. The `*_submit.sh` wrappers resolve sibling job scripts via `$(dirname $0)`, so they keep working unchanged inside `jobs/`.
- **GPU jobs** — dispatched over SSH to host `vancouver01`, activating conda env `klempt_fem` (see `jobs/run_hamilton_kegg_gpu.sh`).
...
- **Keep the repo root lean.** The root deliberately holds only: the ~20 shared `*.py` modules (every one is imported elsewhere — moving any breaks a bare or `nife.` import), `workflow.nf`, and `dieckow_manifest.tsv` (path-referenced by ~6 scripts). The `qsub` job scripts now live in `jobs/`. Everything else belongs in a subdir: reference papers → `docs/refs/` (gitignored), email drafts → `docs/correspondence/`, internship paperwork → `intern_doc/`. Don't leave loose PDFs/zips/logs at root.
