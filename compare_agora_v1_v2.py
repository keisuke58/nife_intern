#!/usr/bin/env python3
"""
Compare AGORA v1 (blood-plasma medium) vs v2 (realistic saliva concentrations).
...
print('\n=== Symmetric (Hamilton) ===')
for name, cat in [('v1', cat_v1_sym), ('v2', cat_v2_sym)]:
...
print('\n=== Directed (gLV) ===')
for name, cat in [('v1', cat_v1_dir), ('v2', cat_v2_dir)]:
    s = count_stats(cat, sym=False)
    print(f'  {name}: L12+={s[1]} L12-={s[2]} AGORA+={s[3]} AGORA-={s[4]} '
          f'cancelled={s[6]} free={s[5]}  '
          f'total_constrained={s[1]+s[2]+s[3]+s[4]}/90')

# ── Diff matrix: where do v1 and v2 disagree? ──────────────────────────────
diff_sym = (cat_v1_sym != cat_v2_sym).astype(int)
diff_dir = (cat_v1_dir != cat_v2_dir).astype(int)

print(f'\nDifferences sym: {diff_sym.sum()} cells changed')
print(f'Differences dir: {diff_dir.sum()} cells changed')

# List changed pairs
print('\n-- Symmetric changed pairs --')
for i in range(N):
    for j in range(i+1, N):
        if cat_v1_sym[i,j] != cat_v2_sym[i,j]:
            print(f'  {SHORT[i]} ↔ {SHORT[j]}: v1={cat_v1_sym[i,j]} → v2={cat_v2_sym[i,j]}')

print('\n-- Directed changed pairs (new competition) --')
for i in range(N):
    for j in range(N):
        if i == j: continue
        if cat_v1_dir[i,j] != cat_v2_dir[i,j]:
            print(f'  {SHORT[j]}→{SHORT[i]}: v1={cat_v1_dir[i,j]} → v2={cat_v2_dir[i,j]}')
...
fig.suptitle('AGORA Medium v1 vs v2 — Sign Prior Coverage\n'
             'v1: blood-plasma scale (~100× too high, all positive)\n'
             'v2: realistic saliva concentrations (MacArthur cosine competition)',
             fontsize=12, fontweight='bold')

fig.tight_layout(rect=[0, 0.07, 1, 0.94])
fig.savefig(OUT / 'fig_agora_v1_v2_comparison.png', dpi=150, bbox_inches='tight')
fig.savefig(OUT / 'fig_agora_v1_v2_comparison.pdf', dpi=150, bbox_inches='tight')
print('\nSaved fig_agora_v1_v2_comparison.{png,pdf}')
