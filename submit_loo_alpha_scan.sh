#!/bin/bash
# submit_loo_alpha_scan.sh — submit LOO-CV alpha scan (3 alphas × 10 folds = 30 jobs)
#
# Usage:
#   bash submit_loo_alpha_scan.sh                  # submit all
#   bash submit_loo_alpha_scan.sh --dry-run        # print commands only
#   bash submit_loo_alpha_scan.sh --alpha 0.5      # single alpha, all 10 folds
#
# Output: nife/results/dieckow_cr/loo_expanded_a{alpha}_fold{k}.json
...
for alpha in "${ALPHAS[@]}"; do
    ...
    for fold in $(seq 0 9); do
        ...
        out_name="loo_expanded${agora_tag}_${alpha_tag}_fold${fold}"

        job_script=$(mktemp /tmp/loo_job_XXXX.sh)
        cat > "$job_script" <<EOF
#!/bin/bash
#PBS -N ${out_name}
