...
SUFFIX = '_noprior' if args.no_prior else ''

# ── Load Duran-Pinedo data ─────────────────────────────────────────────────────────
phi_all = np.load(DATA / 'phi_guild.npy')     # (15, 7, 10)
meta    = json.load(open(DATA / 'metadata.json'))
N_P, N_T = phi_all.shape[:2]
print(f'Duran-Pinedo: {N_P} patients × {N_T} timepoints × {N_G} guilds', flush=True)
...
A_duranpinedo = res.x[:N_G * N_G].reshape(N_G, N_G)
b_duranpinedo = res.x[N_G * N_G:].reshape(N_P, N_G)

# ── Load Dieckow A ────────────────────────────────────────────────────────────
d_dk = json.load(open(CR / 'fit_glv_hamilton_kegg_expanded_agora_w1p0.json'))
A_dk = np.array(d_dk['A'])

# ── Sign agreement: Duran-Pinedo A vs Dieckow A ────────────────────────────────────
print('\n── Sign agreement: Duran-Pinedo A vs Dieckow A ──────────────────────')
...
def sign_agree(pairs, A1, A2, label=''):
    ...
    p_binom = stats.binomtest(m, n, 0.5, alternative='greater').pvalue if n > 0 else 1.0
    print(f'  {label}: {m}/{n} = {m/n:.1%}  p={p_binom:.4f} (binomial vs 50%)')
    return m, n, matches

m_all,  n_all,  _ = sign_agree(pairs_all,         A_duranpinedo, A_dk, 'All 45 pairs')
m_con,  n_con,  _ = sign_agree(pairs_constrained, A_duranpinedo, A_dk, f'Constrained ({n_pairs})')
# Strong pairs only (|A| > 0.1 in BOTH matrices)
strong = [(i,j) for i,j in pairs_all
          if abs(A_duranpinedo[i,j]) > 0.10 and abs(A_dk[i,j]) > 0.10]
if strong:
    m_str, n_str, _ = sign_agree(strong, A_duranpinedo, A_dk,
                                  f'Strong in both (|A|>0.1, n={len(strong)})')

# ── Per-pair table ────────────────────────────────────────────────────────────
print('\n── Top 10 magnitude pairs in Dieckow A (vs Duran-Pinedo A) ──────────')
dk_pairs_sorted = sorted(pairs_all, key=lambda p: abs(A_dk[p[0], p[1]]), reverse=True)
for i, j in dk_pairs_sorted[:10]:
    agree = '✓' if np.sign(A_dk[i,j]) == np.sign(A_duranpinedo[i,j]) else '✗'
    c_str = '●' if mask[i, j] else ' '
    print(f'  {c_str} {GUILD_ORDER[i][:12]:12} ↔ {GUILD_ORDER[j][:12]:12}  '
          f'Dieckow={A_dk[i,j]:+.3f}  Duran-Pinedo={A_duranpinedo[i,j]:+.3f}  {agree}')

# ── Training RMSE of Duran-Pinedo fit ──────────────────────────────────────────────
sq, cnt2 = 0.0, 0
for p in range(N_P):
    phi_pred = phi_all[p, 0].copy()
    for t in range(1, N_T):
        phi_pred = integrate_step(phi_pred, b_duranpinedo[p], A_duranpinedo)
        sq += np.sum((phi_pred - phi_all[p, t])**2)
        cnt2 += N_G
rmse_duranpinedo = np.sqrt(sq / cnt2)
print(f'\nDuran-Pinedo fit RMSE (in-sample): {rmse_duranpinedo:.4f}', flush=True)

# ── Save ──────────────────────────────────────────────────────────────────────
result = {
    'A_duranpinedo': A_duranpinedo.tolist(),
    ...
    'duranpinedo_rmse': float(rmse_duranpinedo),
...
json.dump(result, open(OUT / f'duranpinedo_A_comparison{SUFFIX}.json', 'w'), indent=2)
print(f'Saved → {OUT}/duranpinedo_A_comparison{SUFFIX}.json', flush=True)
...
for ax, A, title in zip(axes,
    ...
    if A is not None:
        ...
        for i in range(N_G):
            for j in range(N_G):
                if i != j:
                    agree_mat[i, j] = 1 if np.sign(A_dk[i,j]) == np.sign(A_duranpinedo[i,j]) else -1
        ...
        ax.set_title(f'(C) Sign agreement: Dieckow vs Duran-Pinedo\n'
                     f'All: {m_all}/{n_all}={m_all/n_all:.0%}  '
                     f'Constrained: {m_con}/{n_con}={m_con/n_con:.0%}',
                     fontsize=9)

fig.suptitle('Cross-dataset A matrix comparison: Dieckow 2024 vs Duran-Pinedo 2021',
             fontsize=11, y=1.02)
fig.tight_layout()
for ext in ('png', 'pdf'):
    fig.savefig(OUT / f'fig_duranpinedo_A_comparison{SUFFIX}.{ext}', dpi=150, bbox_inches='tight')
    print(f'Saved {OUT}/fig_duranpinedo_A_comparison{SUFFIX}.{ext}')
