...
def main() -> int:
    ...
    ap.add_argument("--workdir", type=Path, default=Path(__file__).resolve().parents[2])
    ap.add_argument("--rep-seqs", type=Path, default=Path(__file__).resolve().parents[2] / "data" / "qiime2_out" / "rep-seqs.qza")
    ap.add_argument(
        "--classifier",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "data" / "classifiers" / "classifier.qza",
    )
    ap.add_argument("--outdir", type=Path, default=Path(__file__).resolve().parents[2] / "data" / "qiime2_taxonomy")
