#!/usr/bin/env python3
"""
loo_nsp_ift_v3_gpu.py — NSP Hamilton ODE LOO-CV v3

Improvements over v2:
  - Alternating A/b optimization (--n-rounds, default 3)
  - maxfun=5000 (from 2000)
  - Warm-start A from fit_guild_hamilton_masked_v2_10g.json (10-guild NSP approx)
  - Round 2+ uses previous round's b as warm-start (not zeros)

Usage:
  CUDA_VISIBLE_DEVICES=0 python3 loo_nsp_ift_v3_gpu.py --fold-start 0 --fold-end 5
  CUDA_VISIBLE_DEVICES=1 python3 loo_nsp_ift_v3_gpu.py --fold-start 5 --fold-end 10
"""
...
def run_loo(phi_all, patients, args):
    ...
    # Warm-start A: prefer 10-guild NSP approx, fall back to earlier fits
    warm_paths = [
    ...
    for wp in warm_paths:
        ...
        b0       = np.array(d.get('b_all', np.zeros((len(stored), N_SP))))
        if b0.ndim == 1:
            b0 = b0.reshape(-1, N_SP)
        if b0.shape[1] < N_SP:
            b0 = np.pad(b0, ((0, 0), (0, N_SP - b0.shape[1])))
        warm_b_map = {p: b0[i] for i, p in enumerate(stored) if i < len(b0)}
    ...
    for fold in fold_range:
        ...
        # Remove test patient from warm_A (train only)
        A_current = jnp.array(warm_A, dtype=jnp.float64)
        b_current = jnp.zeros((n_train, N_SP), dtype=jnp.float64)

        # ── Alternating rounds of A/b optimization ────────────────────────
        for rnd in range(args.n_rounds):
            verbose_rnd = (fold == args.fold_start and rnd == 0)

            # Step A: optimize A_upper with current b
            A_fun = _jax_to_scipy(vg_A, b_current, phi_train_j, mask_train_j,
                                  verbose=verbose_rnd)
            res_A = minimize(
                A_fun,
                x0=np.array(A_current),
                method='L-BFGS-B',
                jac=True,
                bounds=a_bounds,
            ...
            A_current = jnp.array(_clip_diag_np(res_A.x), dtype=jnp.float64)
            print(f'  Round {rnd+1} StepA: loss={res_A.fun:.5f}  iters={res_A.nit}'
                  f'  ({time.time()-t_fold:.1f}s)', flush=True)

            # Step b: optimize b_train with current A
            # Round 0: start from zeros (alpha=100 makes b_warm gradient too large)
            # Round 1+: warm-start from previous round's b
            x0_b = (np.zeros(n_train * N_SP) if rnd == 0
                    else np.array(b_current).ravel())
            b_fun = _jax_to_scipy(vg_b, A_current, phi_train_j, mask_train_j,
                                  verbose=verbose_rnd)
            res_b = minimize(
                b_fun,
                x0=x0_b,
            ...
            b_current = jnp.array(res_b.x.reshape(n_train, N_SP), dtype=jnp.float64)
            print(f'  Round {rnd+1} Stepb: loss={res_b.fun:.5f}  iters={res_b.nit}'
                  f'  ({time.time()-t_fold:.1f}s)', flush=True)
        ...
        # ── Step 3: Refit b_test ──────────────────────────────────────────
        bt_fun = _jax_to_scipy(vg_bt, A_opt, phi_test_j, mask_test_j,
    ...
    print(f'\n{"="*60}', flush=True)
    print(f'NSP IFT v3 LOO mean RMSE (folds {args.fold_start}-{args.fold_end-1}): {mean_rmse:.5f}', flush=True)
...
def main():
    ...
    ap.add_argument('--n-rounds',   type=int,   default=3,
                    help='Number of alternating A/b optimization rounds per fold')
    ...
    if args.out_json is None:
        ...
        args.out_json = str(_here / 'results' / 'dieckow_cr' /
                            f'loo_nsp_ift_v3_{tag}.json')

    print(f'JAX devices: {jax.devices()}', flush=True)
    print(f'n_steps={args.n_steps}  dt={args.dt}  n_rounds={args.n_rounds}', flush=True)
    ...
    json.dump({
        ...
        'per_patient':         merged,
        'model':               (f'NSP Hamilton IFT v3 LOO-CV '
                                f'(n_rounds={args.n_rounds}, n_steps={args.n_steps}, '
                                f'dt={args.dt}, maxfun={args.maxfun})'),
