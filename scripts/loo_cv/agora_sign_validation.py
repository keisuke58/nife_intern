...
AGORA_DIR = Path(__file__).resolve().parents[2] / 'data' / 'homd_db' / 'agora_gems'
GLV_JSON  = Path(__file__).resolve().parents[2] / 'results' / 'dieckow_cr' / 'fit_glv_8pat_kegg_prior.json'
OUT_JSON  = Path(__file__).resolve().parents[2] / 'results' / 'dieckow_cr' / 'agora_sign_validation.json'
FIG_DIR   = Path(__file__).resolve().parents[3] / 'docs' / 'figures' / 'dieckow'
...
def main():
    ...
    if not GLV_JSON.exists():
        ...
        alt = Path(__file__).resolve().parents[2] / 'results' / 'dieckow_cr' / 'fit_guild_hamilton_masked.json'
        if alt.exists():
            GLV_JSON_use = alt
        else:
            sys.exit(1)
