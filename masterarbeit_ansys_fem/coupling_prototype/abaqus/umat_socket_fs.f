c =====================================================================
c  umat_socket_fs.f — finite-strain (NLGEOM) self-contained socket UMAT.
c  Sends the deformation gradient DFGRD1 (not small strain) to the
c  finite-strain server umat_server_nsp_fs.py:
c     send : NSTATV  DFGRD1(9,row-major)  STATEV(NSTATV)  E  nu \n
c     recv : STRESS(6) DDSDDE(36) STATEV_new(NSTATV)
c  Server 127.0.0.1:50007. Use with *DEPVAR 12 and NLGEOM=YES.
c =====================================================================
      module bridge_mod_fs
      use iso_c_binding
      implicit none
      interface
...
c ---------------------------------------------------------------------
     ...
     3 DFGRD0(3,3),DFGRD1(3,3)
      ...
      if (irc /= 0) then
        write(*,*) 'UMAT_FS bridge_call failed rc=', irc
        call XIT
      end if
      do i=1,6
        STRESS(i) = resp(i)
