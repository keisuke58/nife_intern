c =====================================================================
c  umat_socket_col.f — finite-strain (NLGEOM) socket UMAT that also sends
c  the Gauss-point COORDS, so the server can apply depth-graded growth.
c     send : NSTATV  COORDS(3)  DFGRD1(9,row-major)  STATEV(NSTATV)  E  nu \n
c     recv : STRESS(6) DDSDDE(36) STATEV_new(NSTATV)
c  Server 127.0.0.1:50007 (umat_server_col.py). *DEPVAR 1, NLGEOM=YES.
c  Requires *USER MATERIAL with the element COORDS available (default for C3D8).
c =====================================================================
      module bridge_mod_col
      use iso_c_binding
      implicit none
      interface
...
c ---------------------------------------------------------------------
     ...
     3 DFGRD0(3,3),DFGRD1(3,3)
      ...
      if (irc /= 0) then
        write(*,*) 'UMAT_COL bridge_call failed rc=', irc
        call XIT
      end if
      do i=1,6
        STRESS(i) = resp(i)
