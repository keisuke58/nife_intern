c =====================================================================
c  umat_socket.f — SELF-CONTAINED Abaqus UMAT that talks to the Python
c  constitutive server over a TCP socket via ISO_C_BINDING (no separate
c  C object to link — avoids the `link_sl` build issue). POSIX libc is
c  linked by Abaqus automatically.
c
c  Protocol (matches umat_server.py / umat_server_nsp.py):
c     send : NSTATV STRAN(6) DSTRAN(6) STATEV(NSTATV) E nu \n
c     recv : STRESS(6) DDSDDE(36 row-major) STATEV_new(NSTATV)
c
c  Server host/port hardcoded to 127.0.0.1:50007 (edit PORTHI/PORTLO).
c  Voigt order = Abaqus (11,22,33,12,13,23). PROPS(1)=E, PROPS(2)=nu.
c =====================================================================
      module bridge_mod
      use iso_c_binding
      implicit none
      ...
      integer function bridge_call(req, nstatev, resp, nresp)
      use iso_c_binding
      implicit none
      character(len=*),intent(in) :: req
      integer,intent(in) :: nstatev, nresp
      double precision,intent(out) :: resp(nresp)
      character(kind=c_char) :: addr(16), rbuf(1)
      character(len=16384) :: rstr
      integer(c_int) :: fd, rc
      integer(c_long) :: nw, nr
      integer :: i, pos, sendlen
      character(len=16384) :: sbuf

      fd = c_socket(2_c_int, 1_c_int, 0_c_int)   ! AF_INET, SOCK_STREAM
      if (fd < 0) then; bridge_call = 1; return; end if

      do i=1,16; addr(i)=char(0); end do
      addr(1)=char(2);  addr(2)=char(0)          ! AF_INET (host order short=2)
      addr(3)=char(195); addr(4)=char(87)        ! port 50007 = htons -> C3 57
      addr(5)=char(127); addr(6)=char(0)
      addr(7)=char(0);  addr(8)=char(1)          ! 127.0.0.1
      rc = c_connect(fd, addr, 16_c_int)
      if (rc /= 0) then; rc=c_close(fd); bridge_call=2; return; end if

      sbuf = trim(req)//char(10)//char(0)        ! append newline
...
c     build request: NSTATV STRAN(6) DSTRAN(6) STATEV E nu
      write(req,'(I0)') NSTATV
      do i=1,6
        write(tmp,'(1X,ES24.16)') STRAN(i); req=trim(req)//trim(tmp)
      end do
      do i=1,6
        write(tmp,'(1X,ES24.16)') DSTRAN(i); req=trim(req)//trim(tmp)
      end do
      do i=1,NSTATV
      ...
      nresp = 6 + 36 + NSTATV
      irc = bridge_call(req, NSTATV, resp, nresp)
      if (irc /= 0) then
        write(*,*) 'UMAT bridge_call failed rc=', irc
        call XIT
      end if
