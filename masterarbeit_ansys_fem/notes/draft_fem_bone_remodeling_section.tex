...
\paragraph{What to measure next (value of information).} The sensitivity ranking is not merely
diagnostic; it is clinically actionable. The two parameters that dominate the 36-month-loss
uncertainty---the osteoclast formation and turnover rates $k_C,g_C$---are precisely the quantities
that a peri-implant crevicular-fluid (PICF) assay reads out: the RANKL:OPG ratio and osteoclast
activity markers (e.g.\ TRAP-5b, CTX). The model therefore makes a concrete recommendation: to
collapse the 90\% credible band into a per-patient risk number, the highest-value measurement is the
PICF RANKL:OPG / osteoclast-marker panel, not a finer mechanical characterisation (the mechanical
synergy $\lambda$ contributes but ranks below the cellular kinetics). This closes the loop from an
abstract sensitivity index to a specific, chair-side biomarker the clinician can order---turning the
model into a guide for which datum most reduces predictive uncertainty.
