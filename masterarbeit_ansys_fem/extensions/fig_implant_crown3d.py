...
def main():
    ...
    # ILLUSTRATIVE crown: borrow the natural-tooth crown (DENTIN, z>30), translate it onto the
    # implant abutment top (z=32.5) and over the implant axis.  Not part of the FEM (no stress).
    ti_c = nodes[conn[mat == "TI"]].reshape(-1, 3); de_c = nodes[conn[mat == "DENTIN"]].reshape(-1, 3)
    didx = np.where(mat == "DENTIN")[0]
    crown_sel = didx[nodes[conn[didx]][:, :, 2].mean(axis=1) > 30.0]
    cfb, _ = boundary_faces(conn[crown_sel])
    abut_top = ti_c[:, 2].max()
    shift = np.array([ti_c[:, 0].mean() - de_c[:, 0].mean(),
                      ti_c[:, 1].mean() - de_c[:, 1].mean(),
                      abut_top - 30.0])
    crown_surf = nodes[cfb] + shift
    crown_top = crown_surf[:, :, 2].max()

    # label anchor points (top of each column, front face)
    yf = b[2] + 0.18 * (b[3] - b[2])
    imp_xyz = (ti_c[:, 0].mean(), yf, crown_top + 2.2)
    ...
    axA.add_collection3d(Poly3DCollection(crown_surf, facecolors=shade(crown_surf, to_rgb(COL["CROWN"]), 0.5, 0.5),
                                          edgecolors=(0, 0, 0, 0.12), linewidths=0.05, rasterized=True))
    label3d(axA, *imp_xyz, "IMPLANT (screw + crown)", "#222222")
    label3d(axA, *too_xyz, "natural tooth", "#b35806")
    axA.set_title(r"(A) restored implant (screw $+$ crown) $+$ natural tooth", fontsize=6.6, pad=-4)
    style3d(axA, b)
    present = [k for k in ["TI", "CROWN", "DENTIN", "PDL", "BIOFILM", "BONE"]
               if (np.isin(mat, [k]).any() or k in ("BONE", "CROWN"))]
    ...
    # illustrative crown: not in the FEM -> render faint grey (no stress)
    axB.add_collection3d(Poly3DCollection(crown_surf, facecolors=(0.78, 0.78, 0.78, 0.35),
                                          edgecolors="none", rasterized=True))
    ...
    fig.savefig(OUT / "fem_implant_crown3d.pdf", bbox_inches="tight", dpi=300)
    fig.savefig(OUT / "fem_implant_crown3d.png", dpi=300, bbox_inches="tight")
    print("wrote fem_implant_crown3d  struct-faces=%d bone-faces=%d vclip=%.2f"
