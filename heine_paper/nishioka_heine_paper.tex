...
\affil[4]{Center for Applied and Computational Mechanics, School of Science for Open and Environmental Systems, Graduate School of Science and Technology, Keio University, Yokohama, Japan}
...
The dominant computational framework for inferring species interactions from microbial time-course data is the generalised Lotka--Volterra (gLV) model~\cite{Stein2013gLV, Bucci2016MDSINE, Venturelli2018Deciphering}, which treats all $n(n+1)$ interaction and growth coefficients as free parameters estimated by point or Bayesian inference.
While gLV has yielded ecological insight in gut and oral communities~\cite{Gonze2018gLV}, it lacks thermodynamic consistency: the asymmetric interaction matrix permits violation of energy balance, and uncertainty quantification is hampered by the high parameter dimensionality relative to the sparse time-series data typical of controlled biofilm experiments.
Building on the extended Hamilton principle~\cite{JunkerBalzani2021ExtendedHamilton}, Klempt et al.~\cite{Klempt2025ContinuumBiofilm} formulated a continuum model in which a \emph{symmetric} interaction matrix~$\mathbf{A}$ governs species growth --- halving the parameter count from 30 to 15 for five species --- with an optional decay vector~$\mathbf{b}$ for antibiotic effects.
...
Despite these advances, estimating the 15-dimensional parameter vector from limited time-course data remains challenging.
...
In dysbiotic states, the model captures the pathogen dynamics with high fidelity: in Phase~1, DH achieves $R^2\geq0.8$ for \textit{An}, \textit{Vei}, and \textit{Pg}; in Phase~2, \textit{Vei} drops to~$0.63$ while \textit{An} ($0.86$) and \textit{Pg} ($0.90$) remain above~$0.8$.
...
Similarly, the \textit{Veillonella} row corresponds to \textit{V.~dispar} in commensal and \textit{V.~parvula} in dysbiotic conditions~\cite{Heine2025PeriImplant}.
...
\textit{Veillonella} species also differ: \textit{V.~dispar} (commensal) and \textit{V.~parvula} (dysbiotic)~\cite{Heine2025PeriImplant}.}
\label{fig:fits_all}
\end{figure}
...
the temporal profile of gingipain (Heine et al.~\cite{Heine2025PeriImplant}, Fig.~4B), the principal virulence factor of \textit{Pg}, correlates strongly with the predicted Pg trajectory in DH, both exhibiting delayed onset at Day~15--21.

\textit{Metabolic sign consistency}:
the MAP \textit{So}$\to$\textit{An} coefficient ($a_{12}=+0.76$) is consistent with lactate/succinate cross-feeding, while \textit{So}$\to$\textit{Vei} ($a_{13}=-2.73$) indicates niche competition dominates---ecologically plausible for species sharing metabolic substrates (Heine et al.~\cite{Heine2025PeriImplant}, Fig.~4C).
...
The authors acknowledge computational support and scientific expertise provided by the Institut für Kontinuumsmechanik (IKM), Leibniz Universität Hannover, and the Lower Saxony Centre for Biomedical Engineering, Implant Research and Development (NIFE), Hannover. Computational resources of the Multiphysics Materials Computation Research Group (Prof.\ M.\ Muramatsu), Department of Mechanical Engineering, Keio University, are also gratefully acknowledged.
