...
Despite these advances, estimating the 15-dimensional parameter vector (15 interaction coefficients of the symmetric $\mathbf{A}$) from limited time-course data remains challenging.
...
\begin{tikzpicture}[
    ...
    >=Stealth,
    box/.style={
        draw, rounded corners=3pt, minimum width=5.0cm, minimum height=0.75cm,
        text centered, fill=white, inner sep=4pt
    },
    databox/.style={
        box, fill=blue!8, draw=blue!50!black
    },
    phase1box/.style={
        box, fill=orange!10, draw=orange!60!black
    },
    phase2box/.style={
        box, fill=red!8, draw=red!50!black
    },
    outbox/.style={
        box, minimum width=3.6cm, fill=green!8, draw=green!50!black
    },
    condbox/.style={
        draw, rounded corners=2pt, minimum width=2.2cm, minimum height=0.6cm,
        text centered, font=\footnotesize, fill=gray!8, draw=gray!50
    },
    arrow/.style={thick, ->},
    label/.style={font=\footnotesize\itshape, midway, right=2pt}
]

% ---- Input data row ----
\node[condbox] (cs) at (-3.4, 0) {CS};
\node[condbox] (ch) at (-1.1, 0) {CH};
\node[condbox] (ds) at ( 1.1, 0) {DS};
\node[condbox] (dh) at ( 3.4, 0) {DH};

% surround with a data box
\begin{scope}[on background layer]
\node[databox, minimum width=9.5cm, minimum height=1.05cm, label={[font=\footnotesize\bfseries]above:Heine et al.\ in-vitro data\ \ ($\phi_i,\psi_i$;\ $N{=}8$ replicates,\ Days 1,3,6,10,15,21)}]
    (data) at (0, 0) {};
\end{scope}

% Phase 1a
\node[phase1box, below=0.65cm of data, minimum width=9.5cm]
    (p1a) {\textbf{Phase 1a:} fix-$\psi$, wide prior \quad $N_p = 1\,000$ particles (GPU \texttt{vmap})};

\draw[arrow] (data.south) -- (p1a.north);

% Phase 1b
\node[phase1box, below=0.45cm of p1a, minimum width=9.5cm]
    (p1b) {\textbf{Phase 1b:} fix-$\psi$, narrowed prior \quad posterior MAP $\hat{\mathbf{A}}_{\text{fix-}\psi}$, $\ln\hat{Z}_1$};

\draw[arrow] (p1a.south) -- (p1b.north)
    node[label] {$q_{0.01}\!-\!1.5\sigma\ \le\ \theta_k\ \le\ q_{0.99}\!+\!1.5\sigma$};

% Phase 2
\node[phase2box, below=0.45cm of p1b, minimum width=9.5cm]
    (p2) {\textbf{Phase 2:} free $\psi$, informative $\mathbf{A}$-prior \quad $N_p = 10\,000$ (GPU RTX\,4090, ${\sim}200\times$ vs CPU)};

\draw[arrow] (p1b.south) -- (p2.north)
    node[label] {Phase\,1b posterior $\to$ $\btheta$-prior};

% Outputs
\node[outbox, below left=0.65cm and 1.5cm of p2]
    (oA) {MAP $\hat{\mathbf{A}}$ per condition\\heatmaps (Fig.~\ref{fig:heatmaps})};

\node[outbox, below=0.65cm of p2]
    (opost) {Posterior violin plots\\15 parameters (Fig.~\ref{fig:posterior})};

\node[outbox, below right=0.65cm and 1.5cm of p2]
    (ofits) {Predictive fits $\phi_i(t)$\\RMSE, $R^2$ (Fig.~\ref{fig:fits_all})};

\draw[arrow] (p2.south) -| (oA.north);
\draw[arrow] (p2.south)  -- (opost.north);
\draw[arrow] (p2.south) -| (ofits.north);

\end{tikzpicture}
\caption{Inference pipeline overview. Input: four in-vitro conditions (commensal/dysbiotic $\times$ static/HOBIC) from Heine et al., digitised species volume fractions $\phi_i$ and viability fractions $\psi_i$. Phase~1 fixes $\psi_i$ to the experimental membrane-intact ratios, runs TMCMC twice (wide then narrowed prior) to identify the symmetric interaction matrix $\mathbf{A}$. Phase~2 releases $\psi_i$ with the Phase~1 posterior as an informative $\mathbf{A}$-prior, yielding the full joint posterior. All particle evaluations use GPU-parallel JAX (\texttt{jax.vmap}) on an NVIDIA RTX\,4090 (${\sim}200\times$ CPU speedup).}
\label{fig:pipeline}
\end{figure}
...
The Heine et al.\ experiments do not involve antibiotic agents, so we set $\alpha^*=0$, which eliminates the decay term $\alpha^*b_i\psi_i$ from Eq.~\eqref{eq:psi_evol}; the antibiotic sensitivity parameters~$b_i$ are unidentifiable and fixed to zero.
The symmetric interaction matrix~$\mathbf{A}$ has 15 independent entries, forming the complete 15-dimensional parameter vector~$\btheta$ (Section~\ref{sec:model_matrix}).
...
Since the Heine et al.\ experiments do not involve antibiotics ($\alpha^*=0$), the antibiotic sensitivity parameters~$b_i$ in Eq.~\eqref{eq:psi_evol} are identically zero and excluded from estimation.
...
All 15 interaction parameters are estimated jointly in a single TMCMC run per condition, enabled by the fix-$\psi$ strategy and GPU acceleration detailed in Section~\ref{sec:gpu}.
...
\midrule
\multicolumn{4}{@{}l}{\textit{Final production (4 cond., $N_p=10{,}000$, $M_{\max}=20$, Phase~2 free $\psi$):}}\\
...
Phase~2 provides the physically consistent model: DS maintains the best fit (RMSE$=$0.033, $R^2_{\mathrm{Pg}}=0.85$), while DH achieves strong pathogen identification ($R^2_{\mathrm{Pg}}=0.90$) despite the additional $\psi$ degrees of freedom.
...
In dysbiotic states, the model captures the pathogen dynamics with high fidelity: DH achieves $R^2>0.8$ for \textit{An}, \textit{Vei}, and \textit{Pg}; DS achieves $R^2=0.93$ for \textit{Vei} and $0.85$ for \textit{Pg}.
...
The antibiotic sensitivity parameters $b_i$ are fixed to zero ($\alpha^*=0$) and excluded from the posterior.
...
While this is a retrospective regression rather than a forward prediction, the negative \textit{So} coefficient is consistent with lactate-driven acidification, and the fact that predicted (not observed) species fractions explain 71\% of pH variance provides indirect support for the inferred interaction dynamics.
...
the MAP \textit{An}$\to$\textit{Vei} coefficient ($A_{23}=+0.76$) is consistent with lactate/succinate cross-feeding, while So$\to$Vei ($A_{13}=-2.73$) indicates niche competition dominates---ecologically plausible for species sharing metabolic substrates (Fig.~4C).
...
\begin{itemize}[nosep,leftmargin=*]
    ...
    \item External validation against independent in-vivo datasets (e.g., Dieckow et al.).
\end{itemize}

%==============================================================================
\bibliographystyle{unsrt}
\bibliography{reference_ikm}
%==============================================================================
