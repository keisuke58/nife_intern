...
ISO 14801）でも**ペリインプラント歯槽頂骨応力（p95）が ×1.5（18→27 MPa）**。
...
| クラウン（リチウムジシリケート / ジルコニア） | 95 000 / 210 000 | 0.30 |
| 歯肉（粘膜） | 3 | 0.45 |
| バイオフィルム（＋成長 $\varepsilon_g=0.19$） | 1.0 | 0.45 |

**境界**：骨の crop 面で完全固定（$U_{1,2,3}=0$）。**界面**：オッセオ＝\*TIE、クラウン＝アバットメント天面に \*TIE、脱統合変種のみ接触 $\mu=0.3$。
**荷重**：ISO 14801、100 N・30°斜め（lat/ax=0.577）。Step1=成長固有ひずみ、Step2=咬合（クラウンありは**咬合面 z=38**＝モーメントアーム）。
**要素**：連成=C3D4（〜27万）、ISO14801 クーポン=C3D10（線形は集中を 9–15% 過小）、収束 ±7%。単位 mm-N-MPa。詳細は `FEM_CONDITIONS.md`。
