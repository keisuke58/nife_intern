...
%% ---------------------------------------------------------------
%%  CZM 説明フレーム (2026-06-29 追記)
%% ---------------------------------------------------------------

\begin{frame}[shrink=15]{Cohesive Zone Model (CZM) とは}
\protect\hypertarget{czm-intro}{}

\begin{columns}[T]
\begin{column}{0.50\textwidth}
\textbf{概念}：き裂先端前方に\textbf{凝着帯}を仮定し，
面間の引き離し力 $T$ と開口変位 $\delta$ の関係（traction-separation 則）で破壊を記述。

\[
T = f(\delta), \qquad
G_c = \int_0^{\delta_c} T\,d\delta
\]

$T$--$\delta$ 曲線下面積 $=$ 破壊エネルギー $G_c$；
$\delta \to \delta_c$ で界面要素が消えることでき裂前進を表現。

\vspace{6pt}
\textbf{特徴}
\begin{itemize}\setlength\itemsep{2pt}
\item Abaqus 標準搭載，実験 $G_c$ と直結 → \textbf{実績最多}
\item き裂位置を\textbf{事前に界面要素として指定}する必要あり
\item 変分的な熱力学基盤なし（$T=f(\delta)$ を直接仮定）
\end{itemize}
...
\toprule
 & CZM & AT2 \\
\midrule
き裂経路 & 事前指定 & $d(\mathbf{x},t)$ 自発 \\
変分基盤 & なし & $\partial\mathcal{E}/\partial d=0$ \\
破壊閾値 & $T=f(\delta)$（別仮定） & $\Psi_e \ge G_c/(2\ell)$ \\
$G_c$ 対応 & 直接 $\int T\,d\delta$ & $\ell$ と組み合わせ \\
実装コスト & 低（標準） & 高（疎行列/反復） \\
\bottomrule
\end{tabular}
}
...
\vspace{4pt}
\textbf{本研究で AT2 を選んだ理由}：
\begin{itemize}\setlength\itemsep{2pt}
\item Klempt の $\Psi_e$ を\textbf{変分的に再利用}（新構成則不要）
\item き裂経路の事前指定不要
\item $t_{\rm crit}$ 比の定性結論は両者で一致
      （$G_c$ キャンセル $\Rightarrow$ CZM でも同値）
\end{itemize}

{\footnotesize CZM は文献最多数派；AT2 は
Klempt フレームとの変分整合性で厳密性が高い。}
\end{column}
\end{columns}
\end{frame}

\begin{frame}[shrink=15]{AT2 phase-field --- Klempt との変分的整合性}
...
\;\to\; \Psi_e \;\xrightarrow{\scriptstyle\Psi_e \ge G_c/(2\ell)}\; d \text{ 核生成}
\]
...
{\footnotesize
\begin{tabular}{@{}lp{2.0cm}p{1.9cm}@{}}
\toprule
 & AT2 & CZM \\
\midrule
...
\begin{frame}[shrink=12]{AT2 正則化長さ $\ell$ の物理的正当化}
...
{\small 文献値から 3 桁ずれるが，$t_{\rm crit}^{\rm DH}/t_{\rm crit}^{\rm CH}=2.00$ は $G_c$ 絶対値に依存しない（キャンセル）$\Rightarrow$ 堅牢。}
\end{column}
\end{columns}
\end{frame}
...
%% ---------------------------------------------------------------
%%  Species-level k_eff decomposition — 4 conditions (2026-06-29)
%% ---------------------------------------------------------------

\begin{frame}[shrink=5]{種レベル $k_{\rm eff}$ 分解 --- 4 条件比較}
\protect\hypertarget{species-keff-4cond}{}

\begin{columns}[T]
\begin{column}{0.50\textwidth}
\textbf{定式化}：
\[
k_{\rm eff}(\phi^*) = \sum_{i=1}^{5}
  \underbrace{\frac{\phi_i^*}{\sum_j \phi_j^*}}_{\text{組成比}}
  \cdot \kappa_i
\]
$\kappa_i$（So/An/Vd/Fn/Pg）$= 1.0,\,0.8,\,0.4,\,0.6,\,0.3$

\vspace{6pt}
\textbf{4条件 TMCMC 後測分布から $n=150$ サンプル積分}：\\[4pt]
{\scriptsize
\begin{tabular}{@{}lccccc@{}}
\toprule
Cond & $k_{\rm eff}$ med & So & An & Vd & Fn/Pg \\
\midrule
...
\item \textbf{CS が最速剥離}（$k_{\rm eff}=0.723$）：
      高 $\kappa$ 種 So+An が共存。
\item \textbf{DS が最遅}（$k_{\rm eff}=0.598$）：
      So が少なく，低 $\kappa$ 種 Fn/An が主体。
\item CH vs DH は\textbf{等価 $k_{\rm eff}$}（$0.637$ vs $0.650$）：
      異なる生態系が同じ力学応答に収束
      （ecological degeneracy in mechanics）。
\item DH では \textbf{Vd が完全消滅}（$= 0.000$）：
      菌叢再編成の fingerprint。
\end{itemize}

\vspace{4pt}
{\footnotesize
$t_{\rm frac}^{({\rm DS})}/t_{\rm frac}^{({\rm CS})}
= k_{\rm eff}^{({\rm CS})}/k_{\rm eff}^{({\rm DS})}
...
%% ---------------------------------------------------------------
%%  生態-力学ブリッジ: λ_max(J) vs t_frac (2026-06-29)
%% ---------------------------------------------------------------

\begin{frame}[shrink=5]{生態的安定性 vs 力学的破壊時刻 --- 4 条件ブリッジ}
\protect\hypertarget{bridge-scatter}{}

\begin{columns}[T]
\begin{column}{0.56\textwidth}
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]%
  {results/figures/fig_replicon_at2_bridge_4cond.pdf}
\end{column}
\begin{column}{0.41\textwidth}
\textbf{左パネル: $\lambda_{\max}(\mathrm{Re}\,J)$ vs $t_{\rm frac}$}\\[4pt]
{\footnotesize
\begin{tabular}{@{}lp{1.5cm}p{1.4cm}@{}}
\toprule
条件 & $\lambda_{\max}$ med & $>0$\,\% \\
\midrule
CH & $0.011$ & $64\%$ \\
CS & $0.011$ & $100\%$ \\
DS & $0.013$ & $100\%$ \\
...
\item \textbf{CH のみ}後測サンプルの $36\%$ が $\lambda<0$
      （生態的安定）— healthy 状態の確率論的頑健性。
\item CS/DS/DH は全サンプル $\lambda>0$
      （限界不安定）— dysbiosis = 安定境界への押し込み。
\item \textbf{生態 $\not\Rightarrow$ 力学}：DH は
      $\lambda_{\max}$ 高いが $k_{\rm eff}$ 小さく破壊は遅い
      （ecological–mechanical trade-off）。
\item 右パネル: $k_{\rm eff}(\phi^*)$ 分布，
      CS が最高（最速剥離）。
\end{itemize}
\end{column}
\end{columns}
...
%% ---------------------------------------------------------------
%%  G_c 逆同定 --- eLife 76027 文献値との整合 (2026-06-29)
%% ---------------------------------------------------------------

\begin{frame}[shrink=5]{破壊エネルギー $G_c$ の逆同定 --- 文献時刻との整合}
\protect\hypertarget{gc-identification}{}

\begin{columns}[T]
\begin{column}{0.56\textwidth}
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]%
  {results/figures/fig_gc_id_lit.pdf}
\end{column}
\begin{column}{0.41\textwidth}
\textbf{逆同定式（AT2 均一場）}：
\[
G_c = (k_{\rm eff}^b \cdot t_{\rm crit})^2 \, E \, \ell
\]
\textbf{適用条件}：$k_{\rm eff}^{\rm phys} \approx 5.67\!\times\!10^{-6}$ s$^{-1}$
...
量 & 値 \\
\midrule
$t_{\rm crit}^{\rm exp}$ & $60\,\mathrm{h}\pm12\,\mathrm{h}$\\
 & (eLife 76027)\\
$G_c^{\rm id}$ & $7.5\pm3.0\ \mathrm{mJ/m}^2$\\
$\Gamma_{\rm lit}$ & $5$--$10\ \mathrm{mJ/m}^2$ \\
 & (eLife 43920)\\
$G_c^{\rm model}$ & $10^{-5}\ \mathrm{J/m}^2$ \\
\bottomrule
\end{tabular}
}
...
\vspace{6pt}
\begin{itemize}\setlength\itemsep{3pt}
\item 逆同定値 $G_c^{\rm id}$ は独立文献 $\Gamma_{\rm lit}$ と整合 $\checkmark$
\item モデル値は 3 桁小さいが，$t_{\rm crit}$ 比は $G_c$ に依存しない
      （$t_{\rm frac}\propto G_c^{1/2}$ がキャンセル）。
\item 不確かさ：$\Delta t/t=20\%\Rightarrow\Delta G_c/G_c=40\%$
      — 実験精度改善が次の課題。
\end{itemize}
\end{column}
\end{columns}
