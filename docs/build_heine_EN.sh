#!/usr/bin/env bash
# build_heine_EN.sh — rebuild ONLY heine_slides_EN (beamer PDF) with the
...
cd "$ROOT"
pandoc "docs/heine_slides_EN.md" -t beamer -s "${EN_FONTS[@]}" -o "docs/heine_slides_EN.tex"
xelatex -interaction=nonstopmode -output-directory=docs "docs/heine_slides_EN.tex" >/tmp/heine_build.log 2>&1
xelatex -interaction=nonstopmode -output-directory=docs "docs/heine_slides_EN.tex" >/tmp/heine_build.log 2>&1
echo "done: $(ls -la "$HERE/heine_slides_EN.pdf")"
