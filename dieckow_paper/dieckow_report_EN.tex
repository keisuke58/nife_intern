...
\paragraph{AGORA medium v2 (realistic saliva).}
The original oral-fluid medium (v1) used nutrient concentrations at blood-plasma scale
($\sim\!100\times$ too high), so all 24 AGORA-added pairs were positive (cross-feeding
dominant, no competition signal).
Medium v2 uses physiologically correct unstimulated saliva concentrations
(Dawes 2008: glucose $\sim$0.08\,mM, total amino acids $\sim$2--5\,mM),
implemented via MacArthur cosine niche-overlap scoring for competition.
Under resource scarcity, competition signals emerge: v2 yields
\textbf{AGORA$+$\,=\,10, AGORA$-$\,=\,14} (symmetric, 45 pairs) and
\textbf{AGORA$+$\,=\,20, AGORA$-$\,=\,17} (directed, 90 pairs), increasing total
constrained pairs from 33/45 to 35/45 (symmetric) and 57/90 to 68/90 (directed).
Notably, the Bacteroidia--Fusobacteriia pair, previously unconstrained (no metabolic
signal in v1), acquires a negative (competition) prior in v2.
LOO-CV comparison v1 vs.\ v2 is a planned next step.
...
\caption{guild-level R/G ratio in 127 Joshi et~al.~(2025) peri-implant samples.
Health/Mucositis near commensal attractor ($\text{R/G ratio}\approx -3$);
Peri-implantitis shifts to dysbiotic attractor ($\text{R/G ratio}\approx 0$).}
\end{figure}

\begin{table}[H]
\centering\small
\caption{guild-level R/G ratio by diagnosis (Joshi 2025, $N=127$).}
\begin{tabular}{lcccc}
\toprule
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Prior mismatch.} L1 is caries-context (eHOMD); transfer to periodontitis
        datasets shows no effect ($<0.1\%$ RMSE, Duran-Pinedo 2021), consistent with disease-state mismatch.
