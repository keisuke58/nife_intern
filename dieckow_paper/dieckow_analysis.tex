...
supplemented by AGORA2 (L2+L3) pFBA cross-feeding predictions — is fit to 16S~rRNA amplicon data from
...
LOO-RMSE~$=0.0504 \pm 0.021$ (AGORA W$=1.0$) vs.\ $0.0516 \pm 0.021$ (L1+L2 only)
and $0.0588$ (unconstrained gLV baseline);
...
$w=1.0$, which acts as a phase transition: below this threshold SA is 94\%, above it
over-constraining reduces the benefit.
LOO stability analysis shows all 45 off-diagonal pairs achieve sign consistency
$\geq 0.70$ across folds (13 pairs at 1.00).
...
\begin{description}[noitemsep]
  \item[Data$+$prior aligned (13 pairs):] $|A_{ij}|>0.3$ \emph{and} $|F_{ij}|>1$.
    Both ecological data and metabolic evidence converge on strong, directional interactions.
    These pairs have sign consistency SC$=1.00$ across all 10 LOO folds.
  \item[Prior-constrained, muted (22 pairs):] $|A_{ij}|<0.01$ \emph{and} $|F_{ij}|>1$.
  ...
  \item[Data-driven, no prior (10 pairs):] $|F_{ij}|=0$ \emph{and} $|A_{ij}|>0.05$.
...
\caption{Biological interpretation of $A$ matrix interaction regimes.
         \textbf{(A)} Scatter plot of prior stiffness $|F_{ij}|$ vs.\ fitted magnitude $|A_{ij}|$
         for all 45 off-diagonal pairs. Colour and symbol indicate regime.
         \textbf{(B)} Network diagram of top-strength pairs with literature annotations.
         Data$+$prior-aligned pairs (red, upper-right) include the three documented
         co-aggregation pairs (Actinobacteria--Bacilli--Betaproteobacteria;~\citealt{kolenbrander2000}).
         Muted pairs (blue, lower-right) include Bacilli--Negativicutes
         (Streptococcus-lactate-Veillonella;~\citealt{mikx1975}) and Fusobacteriia--Actinobacteria
         ($A=-0.08$, negative, consistent with temporal niche separation).}
...
The AGORA W$=1.0$ model reduces LOO-RMSE by 2.4\% over L1+L2 (from 0.0516 to 0.0504)
and by 14.3\% over the unconstrained gLV baseline (0.0588).
The LOO-BC of 0.147 is 5.3\% below L1+L2 (0.155), 4.5\% below gLV free (0.154),
and 47.7\% below the persistence null (0.281).

The RMSE and BC metrics show consistent but not identical rankings: gLV free achieves
RMSE~$=0.0588$ (worse than Hamilton L1+L2) but BC~$=0.154$ (comparable to L1+L2~$=0.155$),
suggesting that gLV free predicts dominant-guild identity correctly but with larger
per-guild magnitude errors. The Hamilton AGORA model is the only variant that improves
...
main models. A paired one-sided $t$-test (AGORA W$=1.0$ vs.\ L1+L2, $N=10$)
gives $p=0.08$, marginally non-significant due to low sample size.
...
\caption{A matrix LOO stability.
         \textbf{(A)} Sign consistency (SC) heatmap across 10 folds (Hamilton + AGORA W$=1.0$).
         All 45 off-diagonal pairs: SC~$\geq 0.70$. Dark red: SC~$=1.00$ (sign robustly
         identified). Pale: SC~$\in[0.70, 0.90]$ (some fold-level sign variation).
         \textbf{(B)} Box-plots of $A_{ij}$ magnitude distributions across folds for
         selected pairs, annotated by regime. Data$+$prior-aligned pairs show tight
         distributions; prior-muted pairs cluster near zero with consistent sign.}
\label{fig:stability}
\end{figure}

Key observations:
\begin{itemize}[noitemsep]
  \item Data$+$prior-aligned pairs (13): SC~$=1.00$ in all 10 folds — both the sign
        and approximate magnitude are robustly identified.
  \item Prior-constrained muted pairs (22): SC~$\geq 0.85$ — the prior fixes the sign
        even when magnitude is near zero, preventing sign-flip artefacts.
  \item Data-driven, no prior (10): SC~$\in[0.70, 1.00]$ — less stable, reflecting
        genuine under-identification in the absence of regularisation.
  \item No pair achieves SC~$< 0.70$ (i.e., no pair flips sign in more than 3/10 folds).
\end{itemize}

The stability analysis provides a fold-level uncertainty proxy and identifies the
13 robustly identified pairs as the model's most reliable output. Pairs with
SC~$< 0.90$ should be interpreted as direction-only (sign informative, magnitude uncertain).
...
Known AGORA L3 limitations include: (1) single representative strain per guild,
ignoring intra-guild metabolic diversity; (2) individual rather than community FBA
(MICOM would be more accurate but circularly depends on community composition);
(3) estimated oral-fluid medium (Dawes 2008~\citep{dawes2008}, not patient-specific);
(4) count-based rather than flux-weighted cross-feeding signals.
Despite these limitations, LOO-CV demonstrates that L3 sign information survives the approximation — the key insight being that sign robustness is inherent to FBA, even under
imprecise medium and representative-strain approximations.
...
\begin{itemize}[noitemsep]
  ...
  \item Robust sign stability (SC~$\geq 0.70$) across all LOO folds.
...
Oral-fluid medium~\citep{dawes2008}: glucose, fructose, sucrose, lactate; 20~amino acids
$+$ Cys, Orn; vitamins B$_1$, B$_2$, B$_3$, B$_5$, B$_6$ (three forms), B$_{12}$;
cofactors (heme, siroheme, adenosylcobalamin, menaquinone, ubiquinone);
meso-DAP (peptidoglycan precursor);
lipids (stearate C18, myristate C14, laurate C12);
trace metals (Fe$^{2+/3+}$, Mg$^{2+}$, Ca$^{2+}$, Zn$^{2+}$, Cu$^{2+}$, Mn$^{2+}$, Co$^{2+}$);
glutathione (ox/red), putrescine, spermidine.
All 10 guilds: $\mu\in[0.11, 1.66]\ \text{h}^{-1}$ (verified).
...
\bibitem[Nishioka et~al.(\textit{in prep.})]{nishioka_paper}
