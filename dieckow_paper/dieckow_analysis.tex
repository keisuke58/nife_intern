...
External validation on 127 independent cross-sectional peri-implant samples~\citep{szafranski2025}
shows that the Dieckow-fitted $\bA$ correctly orders community dysbiosis across health,
mucositis, and peri-implantitis (Kruskal-Wallis $p<0.0001$, Spearman $\rho=0.46$,
binary Health vs.\ PI $\rho=0.59$).
A Hamilton Normal-Species-Pool (NSP) formulation achieves LOO-RMSE~$=0.091$
($1.9\times$ worse than gLV), with the gap attributed to indirect composition
observables and latent-variable warm-up cost.
MDSINE2 (Bayesian gLV) collapses to A$\approx$0 under this data regime
(3 timepoints per patient), yielding LOO-RMSE~$=0.055$ ($12\%$ worse than
gLV $\alpha{=}0.25$), confirming the advantage of sign-prior regularisation
over Bayesian sparsity-induction when $T$ is small.
...
Here we use this three-layer prior to regularise both a symmetric Hamilton replicator
ODE~\citep{klempt2024, klempt2025} and an asymmetric generalised Lotka-Volterra (gLV)
model fit to the Dieckow~et~al.~(2024) longitudinal dataset.
We evaluate out-of-sample generalisation by leave-one-patient-out (LOO) cross-validation
using RMSE and Bray-Curtis dissimilarity, compare a Hamilton Normal-Species-Pool (NSP)
formulation as a mechanistically richer alternative, validate the inferred interaction
matrix on an independent peri-implant dataset~\citep{szafranski2025}, and extend the
fitted model to a 1D reaction-diffusion PDE to probe predicted spatial stratification
along the biofilm depth axis.
...
$A_{ij}$ (no sign constraint, $\calP=0$) using \texttt{scipy.integrate.solve\_ivp}.
...
\subsubsection{Hamilton Normal-Species-Pool (NSP)}
\label{sssec:nsp}

As a mechanistically richer alternative, we test the Hamilton Normal-Species-Pool
model~\citep{junker2021}, in which each guild $i$ carries an extended state
$(\phi_i, \phi_{0,i}, \psi_i, \gamma_i)$ encoding both relative abundance and
environmental normalisation variables. The NSP free-energy formulation gives a
more complex fitness function $f_i^{\text{NSP}}$ that depends on nutrient
concentrations and a shared carrying capacity scalar $\psi$; full equations are
given in~\citet{klempt2024, klempt2025}.

We optimise the NSP via a three-step iterative scheme (IFT~v2):
(i)~fit shared $A_{\text{upper}}$ (55 parameters, triangular) on all training patients;
(ii)~fix $A$, fit per-patient $b^{(p)}$ (9$\times$10 parameters);
(iii)~fit the held-out patient's $b^{(\text{ho})}$ (10 parameters) and predict
W2, W3 via 2-step integration.
Gradients are computed through the fixed-point condition using the implicit function
theorem (IFT): $\nabla_\theta \mathcal{L} = -(\partial G/\partial x)^{-T}\partial G/\partial\theta$,
where $G(x;\theta)=0$ is the steady-state equation.
$b$ initialisation is from zero (warm-start from a previous $b$ estimate was found
to cause gradient blow-up due to $|\nabla|\approx 187$ at the warm-start).

\subsection{Sign prior construction}
\label{ssec:prior}

Figure~\ref{fig:pipeline} shows the full three-layer sign constraint construction pipeline.
We build a net-flow matrix $F_{ij}$ from three evidence layers:

\begin{figure}[htb]
\centering
\includegraphics[width=\linewidth]{fig0b_agora_pipeline.pdf}
\caption{AGORA2 sign constraint construction pipeline.
...
\subsection{External validation: Szafra\'{n}ski peri-implant attractor analysis}
\label{ssec:szafranski_attractor}

Each cross-sectional sample from~\citet{szafranski2025} (127 samples: Health/Mucositis/
Peri-implantitis) is mapped to the 10-guild simplex: the 5 measured genera
(Actinomyces, Streptococcus, Porphyromonas, Fusobacterium, Veillonella) are assigned
to their respective guilds; the remaining 5 guilds are zero-imputed ($\phi=10^{-5}$)
so that the signal is carried entirely by the observed 5 genera.  The Guild Dysbiosis
Index is computed \emph{directly} from the observed composition:
\begin{equation}
  \text{R/G ratio} = \log\!\bigl(\phi_\text{Fuso}+\phi_\text{Bact}+\phi_\text{Clos}\bigr)
             - \log\!\bigl(\phi_\text{Bac}+\phi_\text{Act}+\phi_\text{Neg}\bigr),
  \label{eq:gdi}
\end{equation}
without ODE integration.  We compared this \emph{compositional} R/G ratio against
model-based variants (Hamilton ODE equilibrium, instant R/G ratio $= -\bphi^\top(\bb+\bA\bphi)$
with the gLV $\alpha=0.25$ interaction matrix); the raw compositional ratio consistently
outperformed ODE-based variants for cross-sectional data, where individual trajectories
are at arbitrary points rather than near attractor equilibria.
...
\textbf{gLV $\alpha$=0.25}     & \textbf{L1+L2}        & \textbf{97\% (32/33)}& \textbf{0.0490} & \textbf{0.016} & ---   & --- \\
...
MDSINE2 (Bayesian gLV) & Bayesian     & ---         & 0.0552$^\dagger$ & 0.019 & --- & --- \\
\bottomrule
\multicolumn{7}{l}{\small $^\dagger$8/10 folds completed; remaining 2 folds excluded (MCMC not converged within 7-day walltime); A$\approx$0 (Bayesian evidence insufficient for $N{=}3$ timepoints).} \\
\end{tabular}
\end{table}
...
Among gLV variants, the asymmetric model with $\alpha=0.25$ (L1+L2 only) achieves the
overall best LOO-RMSE ($0.0490 \pm 0.016$), demonstrating that the additional degrees
of freedom in an asymmetric $A$ outweigh the benefit of the sign constraint for the gLV form.
Adding the MICOM L3 layer — whether at $\alpha=0.0$ ($0.0519$) or $\alpha=0.25$
($0.0516$, 100\% SA) — improves sign agreement to perfect (72/72) but increases
RMSE by $5$--$6\%$ relative to gLV $\alpha{=}0.25$ (L1+L2), suggesting that the additional
directed constraints impose a cost in predictive accuracy not compensated by the
biological consistency gain for this dataset size.
...
\textbf{MDSINE2 Bayesian gLV baseline.}
As a fully Bayesian reference, we ran MDSINE2~\citep{gibson2021} (MCMC, burnin~$=500$,
$N_\text{samples}{=}2500$).  With $N{=}9$ training patients and only 3 timepoints each
(27 measurements), the Bayesian model assigns near-zero posterior weight to all interactions
($\bar{I}_{ij}~{<}~0.1\%$), reducing to a growth-rate-only predictor.
LOO-RMSE (8/10 folds) $= 0.0552\pm0.019$, $12.7\%$ worse than
gLV $\alpha{=}0.25$ ($0.0490$).
This confirms that sign-prior regularisation outperforms Bayesian sparsity-induction
when the number of timepoints per patient is small.
...
\caption{A matrix LOO stability (gLV $\alpha$=0.25, best model, 10 folds).
         \textbf{(A)} Sign consistency (SC) heatmap; boxed cells have a non-zero sign constraint.
...
as a function of AGORA weight $w_{L3}$. A abrupt transition at $w=1.0$ achieves
...
\caption{AGORA weight sweep ($w_{L3}\in\{0,0.5,1.0,1.5,2.0\}$).
         ...
         A discrete abrupt transition at $w=1.0$: SA jumps from 94\% to 100\%
...
Table~\ref{tab:joshi} and Figure~\ref{fig:joshi} show R/G ratio distributions across
diagnostic groups in the Joshi dataset.  Health and Mucositis have similar R/G ratio
(mean $-2.09$ vs.\ $-1.74$); Peri-implantitis shows a large positive shift
(mean $+0.53$, median $+0.76$).  Kruskal-Wallis $H=34.7$, $p<0.0001$;
Mann-Whitney Health $<$ PI $p<0.0001$; Spearman
$\rho(\text{diagnosis severity}, \text{R/G ratio}) = 0.46$, $p=5\times10^{-8}$
($N=127$).  Restricting to the binary Health vs.\ Peri-implantitis contrast
($N=88$) yields $\rho=0.59$, $p=2\times10^{-9}$.
The moderate three-way $\rho$ reflects the biological proximity of Health
and Mucositis in the 5-genus composition space (both lack substantial
Porphyromonas/Fusobacterium enrichment); additional pathogens
(Treponema, Tannerella, Prevotella) would be required to resolve early-stage
dysbiosis.

\begin{table}[htb]
\centering
\caption{guild-level R/G ratio by diagnosis (Joshi et~al.\ 2025, 127 samples).
         R/G ratio $= \log(\phi_{\text{dys}}) - \log(\phi_{\text{com}})$ evaluated at
         observed composition (zero-imputation for undetected guilds).}
\label{tab:joshi}
\begin{tabular}{lcccc}
\toprule
Diagnosis & $n$ & Mean R/G ratio & Median R/G ratio & vs.\ Health \\
\midrule
Health           & 56 & $-2.09$ & $-1.88$ & --- \\
Mucositis        & 39 & $-1.74$ & $-1.65$ & $p=0.47$ \\
Peri-implantitis & 32 & $+0.53$ & $+0.76$ & $p<0.0001$ \\
\bottomrule
\multicolumn{5}{l}{\small Kruskal-Wallis: $H=34.7$, $p<0.0001$.
Spearman $\rho=0.46$, $p=5\times10^{-8}$.}
\end{tabular}
\end{table}
...
\caption{External validation: guild-level R/G ratio in Joshi~et~al.~(2025) peri-implant
         samples. \textbf{(A)} Violin + strip plot of R/G ratio by diagnosis.  Health and
         Mucositis overlap substantially; Peri-implantitis shows a strong positive shift
         (R/G ratio $>0$).  \textbf{(B)} Comparison of R/G ratio variants: the raw compositional
         log-ratio (ρ$=0.46$) outperforms ODE-equilibrium variants (ρ$=0.37$--$0.39$)
         because cross-sectional samples are not at attractor equilibria.
...
the symmetry constraint, combined with the sign constraint, provides net regularisation
...
\subsection{gLV versus Hamilton ODE: role of the model class}
\label{ssec:disc_glv_nsp}

The unconstrained asymmetric gLV achieves the best LOO-RMSE overall ($0.0490$, cf.\
Hamilton+MICOM $0.0502$), and the non-symmetric free parameters allow it to capture
directed asymmetries (e.g.\ \textit{Streptococcus} $\text{H}_2\text{O}_2$ inhibition)
that the symmetric $A$ suppresses.
We also tested a Hamilton Normal-Species-Pool (NSP) formulation in which the replicator
is derived from a free-energy quadratic over guild abundances \emph{and} environmental
metabolite pools; the best NSP variant (three-step IFT optimisation) achieves
LOO-RMSE~$=0.091$, roughly $1.9\times$ worse than gLV.

Three mechanisms explain the gap:
\begin{enumerate}[noitemsep]
  \item \textbf{Indirect observables.} The NSP observes $\bar\phi=\phi\psi$ (abundance
        weighted by normalisation scalar $\psi$) rather than $\phi$ directly, so
        the observation gradient in the IFT backward pass is attenuated by $\psi$.
  \item \textbf{Latent warm-up.} The NSP latent state $(\phi_0, \psi, \gamma)$ must
        equilibrate from week~1 before useful predictions emerge; with only two
        prediction steps (W1$\to$W2, W2$\to$W3), the latent dynamics are partially
        unresolved.
  \item \textbf{Parameter count.} The NSP $b$-diagonal enters only the $\psi$-equation,
        making it less directly constrained by $\phi$ observations; the gLV $b_i$
        enters the replicator fitness directly.
\end{enumerate}
The NSP formulation remains scientifically relevant for two reasons.
First, it preserves the Extended Hamilton Principle and admits a free-energy
interpretation of community structure~\citep{junker2021}.
Second, because the full NSP state encodes environmental resource pools,
it can in principle predict multi-stable attractors — a regime where gLV and NSP
predictions diverge qualitatively, not merely quantitatively.
For the present 10-patient, 3-timepoint dataset the gLV wins on predictive accuracy;
whether the NSP advantage on interpretability translates to a predictive edge in
richer datasets remains an open question.

\subsection{Spatial structure: 1D reaction-diffusion PDE outlook}
\label{ssec:disc_pde}

Oral biofilms are spatially stratified: early colonisers (Actinobacteria, Streptococci)
attach to the enamel surface, while late colonisers (Fusobacteria, Negativicutes) occupy
the outer biofilm layers~\citep{kolenbrander2010}.
The LOO $A$ matrix quantifies these interactions in a well-mixed (0D) model, but the
differential enrichment it implies can be tested in a 1D depth model.

We extended the fitted gLV to a reaction-diffusion PDE on normalised depth $z\in[0,1]$:
\begin{equation}
  \frac{\partial\phi_i}{\partial t}
  = D_i\frac{\partial^2\phi_i}{\partial z^2}
  - u\frac{\partial\phi_i}{\partial z}
  + \phi_i\!\left(b_i + \sum_j A_{ij}\phi_j - \langle f\rangle\right),
\end{equation}
with no-flux BC at $z=0$ (substratum) and Dirichlet $\phi_i=\bar\phi^{\,\text{train}}_i$
at $z=1$ (bulk).
Running the model for all 10 LOO folds with fold-specific $A$, training-mean $\phi_{\text{bulk}}$
and $b$, we find that Actinobacteria and Bacilli are consistently enriched near
the substratum ($\Delta\phi_{\text{sub}-\text{bulk}} = 0.115\pm0.100$ and $0.078\pm0.120$,
respectively), while Negativicutes and $\gamma$-Proteobacteria are consistently depleted
($-0.050\pm0.006$ and $-0.037\pm0.005$). This predicted stratification is qualitatively
consistent with the known successional ecology of dental plaque.
$\beta$-Proteobacteria (Neisseria/Eikenella) show the highest fold-to-fold variance
($0.006\pm0.200$), reflecting sensitivity of this guild's substratum affinity to
which patient is held out — suggesting an identifiability limit of the current data
(Figures~\ref{fig:pde_gradient}--\ref{fig:pde_profiles}, Appendix~\ref{app:pde}).

These results are preliminary: diffusivities $D_i$ are placeholders (pending
$z$-resolved CLSM depth profiles from Szafra\'{n}ski lab), and the advection
velocity $u$ represents biofilm surface detachment without direct calibration.
When $z$-profile data become available, the PDE framework can be fitted to depth
gradients directly, providing a quantitative test of whether the inferred $A$
correctly predicts spatial community structure.
...
The Mucositis--Health equivalence at equilibrium (R/G ratio difference~$=0.15$, $p=0.61$)
...
\subsection{Guild-level keystone analysis}
\label{ssec:keystone}

To characterise which guilds structurally anchor the fitted community, we performed
in-silico guild knockout simulations using the best-fit gLV $A$ matrix and
patient-mean growth vector $\bar{b}$ (mean over 10 patients), initialised at
the observed week-0 mean composition. Each guild $i$ was removed by setting
$\phi_i(0)=0$ and zeroing all interactions with guild $i$; the community was then
simulated forward 21 days, and four metrics were computed relative to the intact
baseline: (i) mean Bray-Curtis (BC) divergence, (ii) productivity drop
($\Delta\langle\phi^\top b\rangle$), (iii) volume growth drop
($\Delta\!\int\!\phi_{\text{total}}\,dt$), and (iv) volume fraction
($\int\phi_i\,dt / \int\phi_{\text{total}}\,dt$).

Two guilds emerge as keystone taxa:

\paragraph{Actinobacteria — volume engine.}
Removing Actinobacteria yields BC divergence~$=0.637$ and volume growth
drop~$=0.846$ (85\% of community volume is lost). Actinobacteria accounts
for 15.7\% of the cumulative abundance integral, but drives disproportionately
more total growth through facilitative coupling with Bacilli
($A_{\text{Acti,Baci}}=+3.43$, the strongest interaction in the matrix)
and Betaproteobacteria ($A_{\text{Beta,Acti}}=+1.74$). This is consistent
with Actinomyces being the archetypal early coloniser that provides surface
receptors for Streptococcal adhesion and metabolic bridging~\citep{kolenbrander2000}.

\paragraph{Bacilli — keystone guild with competitive suppression.}
Removing Bacilli produces the highest BC divergence (0.839), yet the volume
growth \emph{increases} by 73\% (volume drop~$=-0.733$). This apparent paradox
reflects competitive suppression: Bacilli (Streptococci) inhibit Betaproteobacteria
($A_{\text{Beta,Baci}}=-2.10$) and are inhibited by Bacteroidia
($A_{\text{Bact,Baci}}=-0.63$). When Bacilli is absent, Betaproteobacteria and
Bacteroidia expand, driving higher total biomass but a qualitatively different
composition. Bacilli is thus a keystone competitor analogous to a dominant
plant species that suppresses community-level productivity through competition,
yet its presence is essential for the Actinobacteria-dominated CT1 state.

\paragraph{Remaining guilds.}
Bacteroidia and Betaproteobacteria show intermediate BC divergence (0.43 and 0.45)
and are primarily structural players within the Fusobacteriia-enriched CT2 state.
Clostridia, Coriobacteriia, Negativicutes, and Other show negligible knockout
effects (BC $<0.05$), consistent with their low observed abundance and
absence of strong prior-supported interactions.

The top-ranked interaction $A_{\text{Acti,Baci}}=+3.43$ substantially exceeds the
next-strongest ($A_{\text{Beta,Baci}}=-2.10$), confirming that the Actinobacteria--Bacilli
mutualism is the backbone of the CT1 commensal state.
Per-patient error bars on BC divergence confirm that these rankings are robust
across patients (SD $< 0.12$ for all guilds; available in the supplementary
JSON output from \texttt{guild\_importance\_analysis.py}).

\subsection{Guild network centrality: influence, vulnerability, and net role}
\label{ssec:network}

Beyond knockout metrics, the structure of the $\bA$ matrix itself encodes
asymmetric ecological roles.  We define for each guild $i$:
\begin{itemize}[noitemsep]
  \item \textbf{Influence}: $\sum_{j\neq i}|A_{ji}|$ --- total effect of guild $i$
        on all other guilds (column-sum of $|\bA|$).
  \item \textbf{Vulnerability}: $\sum_{j\neq i}|A_{ij}|$ --- total effect of all
        other guilds on guild $i$ (row-sum of $|\bA|$).
  \item \textbf{Net effect}: $\sum_{j\neq i} A_{ij}$ --- whether guild $i$ is
        net-facilitated ($>0$, mutualist) or net-inhibited ($<0$, competitor).
\end{itemize}

Results are summarised in Table~\ref{tab:network}.
Actinobacteria has the highest influence ($\sum|A_{ji}|=5.58$) and is
net-facilitated ($+2.02$): it both drives and benefits from the commensal community.
Bacilli has the highest vulnerability ($\sum|A_{ij}|=8.24$) despite moderate
influence (1.57), reflecting its position as the target of several strong
interactions (Actinobacteria $A=+3.43$, Betaproteobacteria $A=-2.10$,
Bacteroidia $A=-0.63$).
Betaproteobacteria is the only guild with both high influence (4.17) and
net-negative effect ($-0.14$), classifying it as the principal competitive
regulator of the commensal state.

\begin{table}[htb]
\centering
\caption{Guild network centrality metrics from the fitted gLV $\bA$ matrix.
...
Betaproteobacteria  & 4.17 & 2.01 & $-$0.14 & \textbf{competitor} \\
...
\subsection{Tipping point analysis: which growth rates govern the dysbiotic transition?}
\label{ssec:tipping}

To quantify which ecological growth conditions govern the CT2 (dysbiotic)
$\to$ CT1 (commensal) transition, we conducted two complementary analyses.

\paragraph{Linear b-interpolation.}
We defined a one-parameter family of growth vectors
$\bb(\alpha) = (1-\alpha)\,\bb_{\text{CT2}} + \alpha\,\bb_{\text{CT1}}$,
where $\bb_{\text{CT1}}$ and $\bb_{\text{CT2}}$ are the per-CT mean growth
vectors from the fitted 10-patient model ($\alpha=0$: full CT2 environment;
$\alpha=1$: full CT1 environment).
For each $\alpha \in [0,1]$, we simulated all 10 patients forward 21 days
from their observed week-0 compositions and computed the week-3 Guild
Dysbiosis Index (R/G ratio~$= \log\phi_{\text{dys}} - \log\phi_{\text{com}}$).
\emph{No value of $\alpha$ drives the mean R/G ratio below zero} (tipping point
$\alpha^*$ does not exist within $[0,1]$).
This confirms the attractor-robustness result: even replacing the full
CT2 growth environment with CT1 growth rates cannot redirect a community
that starts from a week-0 CT2-like composition, because the $\bA$ matrix
determines the equilibrium landscape irrespective of $\bb$.

\paragraph{Single-guild b sweep.}
To identify which individual growth rates most strongly shift R/G ratio, we swept
each $b_i$ from its CT2 to CT1 value while holding all others at CT2 levels.
The two largest R/G ratio drivers are Gammaproteobacteria
($|\Delta\text{R/G ratio}|=1.16$; $\Delta b = -5.31$, i.e.\ CT1 patients have
substantially lower Gammaproteobacteria growth rate) and Bacteroidia
($|\Delta\text{R/G ratio}|=0.93$; $\Delta b = -0.96$).
Reducing Gammaproteobacteria growth rate (e.g.\ via substrate limitation
or targeted antimicrobials) has the largest single-guild effect on shifting
the community toward a commensal R/G ratio — despite Gammaproteobacteria having
only moderate network influence (1.16, Table~\ref{tab:network}).
This dissociation between network influence and growth-rate sensitivity
reflects the difference between structural importance (mediated by $\bA$)
and environmental modulation (mediated by $\bb$): interventions acting
on the ecological niche of Gammaproteobacteria and Bacteroidia offer the
most tractable entry points for growth-rate-mediated dysbiosis reversal.

\subsection{Predicted relapse dynamics: transient recovery vs.\ permanent commensalism}
\label{ssec:relapse}

A direct clinical implication of the fitted gLV dynamics is the predicted
long-term fate of each patient after the 3-week observation window.
We integrated the patient-specific gLV ($\bA$, $\hat{\bb}^{(p)}$, week-0 $\phi_0^{(p)}$)
for 90 days and tracked the guild-level R/G ratio.

Three distinct trajectories emerge (Figure~\ref{fig:relapse}):

\begin{enumerate}[noitemsep]
  \item \textbf{Persistently dysbiotic (6/10 patients: A, B, E, F, H, K):}
        R/G ratio $>0$ already at week 3 and remains positive.
        These patients' $\hat{b}^{(p)}$ vectors have elevated dysbiotic guild growth
        rates that maintain a dysbiotic attractor despite the shared $\bA$.

  \item \textbf{Transient responders (3/10 patients: C, G, L):}
        R/G ratio $<0$ at week 3 (apparent commensal recovery), but the model predicts
        a R/G ratio crossing to positive at days 24, 38, and 48, respectively.
        Patient L shows the most striking profile: R/G ratio~$=-5.66$ at week 3
        (deepest apparent recovery) yet relapses at day 48.
        This suggests that the week-3 commensal state is a transient deflection
        imposed by the initial condition, not a stable attractor under their
        patient-specific growth dynamics.

  \item \textbf{Permanent commensal (1/10 patients: D):}
        R/G ratio remains $<0$ throughout, converging to a stable commensal equilibrium.
        Patient~D (CT1) is the only patient whose $\hat{b}^{(p)}$ vector supports
        a commensal long-time attractor under the inferred $\bA$.
\end{enumerate}

This patient-level heterogeneity in predicted long-term fate emerges entirely
from the patient-specific $\hat{\bb}^{(p)}$ vectors: all patients share the
same $\bA$ matrix, yet most converge to dysbiosis at long times because their
fitted growth rates encode a dysbiotic ecological niche.
This is consistent with the clinical reality of caries recovery: dental
intervention (scaling, prophylaxis) temporarily disrupts the dysbiotic
community (transient R/G ratio reduction) but does not permanently alter the
patient's underlying oral environment.
Long-term commensalism requires a lasting shift in $b_i$ --- for instance,
through diet modification (reducing fermentable substrate availability,
which directly affects $b_{\text{Bact}}$ and $b_{\text{Fuso}}$) or immune-mediated
changes in gingival crevicular fluid composition.

\textbf{Caveat:} these predictions extend substantially beyond the 3-week
calibration window and should be interpreted as qualitative extrapolations,
not validated clinical forecasts. Validation requires longitudinal follow-up
data beyond week 3 (unavailable in the current Dieckow dataset).

\begin{figure}[htb]
\centering
\includegraphics[width=0.90\linewidth]{relapse_dynamics.pdf}
\caption{Predicted relapse dynamics: R/G ratio trajectories for all 10 patients
         simulated to 90 days using patient-specific gLV
         ($\bA$, $\hat{\bb}^{(p)}$, week-0 $\phi_0^{(p)}$).
         Green shading: 3-week data window. Dashed line: R/G ratio~$=0$ boundary
         (below = commensal, above = dysbiotic).
         \textbf{Blue}: Patient~D (permanent commensal, never relapses).
         \textbf{Orange}: Transient responders C, G, L (commensal at week 3,
         relapse at days 24, 38, 48).
         \textbf{Red/light-blue}: 6 persistently dysbiotic patients.
         Note: predictions beyond week 3 are speculative and require
         longitudinal validation.}
\label{fig:relapse}
\end{figure}

\subsection{In-silico intervention and attractor robustness}
\label{ssec:intervention}

A key translational question is whether targeted manipulation of initial guild
composition (e.g.\ probiotic supplementation or antibiotic suppression)
can redirect a dysbiotic community toward a healthy reference state.
We formalised this as an in-silico scan: for each of the 10 guilds, we applied
either (i) suppression — $\phi_i(0) \leftarrow \phi_i(0)\,(1-f)$ for
$f\in\{0.1,\ldots,0.9\}$, representing antibiotics or phage therapy — or
(ii) boost — $\phi_i(0) \leftarrow \phi_i(0) + \delta\,\phi_\text{total}(0)$
for $\delta\in\{0.05,\ldots,0.5\}$, representing probiotic supplementation.
After renormalisation, each perturbed community was simulated forward 21 days
with the patient-specific gLV $A$ and $b^{(p)}$, and the Bray-Curtis distance
of the simulated week-3 composition to the healthy reference
(mean observed week-3 across all 10 patients) was recorded.

The baseline BC to the healthy reference is
$0.992 \pm 0.019$ (mean $\pm$ SD across patients), close to the theoretical
maximum of 1.0 for two maximally dissimilar compositions.
The best single-guild intervention is 90\% suppression of Bacteroidia
($\Delta$BC~$=+0.002$); boosting Gammaproteobacteria at $\delta=0.5$
achieves $\Delta$BC~$=+0.001$.
Even the most aggressive compositions change reduces the BC to the healthy
reference by less than $0.3\%$.

This near-zero intervention sensitivity reveals a fundamental property of the
fitted gLV: the dynamical attractor is overwhelmingly determined by the
interaction matrix $A$ and growth rates $b$, not by the initial composition.
Any initial perturbation in $\phi(0)$ is rapidly washed out as the system
relaxes toward the $A$- and $b$-determined equilibrium within $\sim$3 weeks.
From a clinical perspective, this implies that the caries recovery observed
in the Dieckow dataset is not achievable through manipulation of the starting
community composition alone; rather, recovery requires a change in the
ecological growth environment — i.e.\ a shift in effective growth rates $b_i$
(e.g.\ through dental cleaning, pH normalisation, or reduced sugar substrate
availability) that redefines which attractor the community converges to.

This interpretation is reinforced by the patient-specific $\hat{b}^{(p)}$ analysis
(Section~\ref{ssec:bvec}): CT1 and CT2 patients differ primarily in their growth
rate vectors, not in $A$, and the $b$-mediated attractor difference is what
distinguishes commensal from dysbiotic equilibria.
A mechanistically grounded intervention strategy would therefore target the
\emph{fitness landscape} (manipulating $b$ via environmental perturbation)
rather than the \emph{initial condition} (manipulating $\phi_0$ via transplantation),
at least within the timescale and data regime of the present study.

\subsection{Relapse prevention threshold and long-term prognosis}
\label{ssec:prevention}

The relapse dynamics (Section~\ref{ssec:relapse}) raise an immediate translational question:
\emph{how large must a clinical change in the ecological growth environment be to convert a transient
responder into a permanent commensal?}
For patients C, G and L — who each showed commensal-like R/G ratio at week 3 but subsequently
re-dysbiosed at days 24, 38, and 48, respectively — we performed a single-guild $b$-sweep:
for each of four candidate target guilds (Bacteroidia, Fusobacteria, Gammaproteobacteria,
Bacilli) we reduced $b_i$ stepwise from its fitted value by up to $\Delta b = 2.0$ and
identified the minimum reduction that eliminates the later R/G ratio sign-change (i.e.\ achieves
permanent commensalism over 150 days).

Only one threshold was identified within the sweep range: for patient~C, reducing the
Bacteroidia growth rate by $\Delta b_\text{Bact} = 1.33$ prevents relapse (converted from
transient to permanent commensal). For patients G and L, no single-guild growth-rate
suppression of up to $\Delta b = 2.0$ was sufficient to eliminate relapse.
This asymmetry reflects patient-specific attractor depths: patient~C has a relatively
shallow dysbiotic basin, whereas patients G and L reside in deeply dysbiotic growth
environments that require multi-guild or large-magnitude intervention.

Taken together, the prevention analysis quantifies the concept of ``attractor depth'':
for most patients the dysbiotic attractor is robust to realistic single-parameter
perturbations, consistent with the near-zero BC improvement found in the initial-condition
intervention scan (Section~\ref{ssec:intervention}).

\paragraph{Week-3 R/G ratio as a prognostic marker.}
Among the three transient responders, the week-3 R/G ratio correlates inversely with relapse day:
patient~L achieves the most commensal-like state at week 3
($\text{R/G}_{w3} = -5.66$, relapse: day 48), patient~C
($\text{R/G}_{w3} = -2.15$, relapse: day 24), and patient~G
($\text{R/G}_{w3} = -1.20$, relapse: day 38).
While $N=3$ precludes statistical inference, this ordering is consistent with the hypothesis
that a more negative week-3 R/G ratio reflects deeper penetration into the commensal basin,
buying additional time before the dysbiotic attractor reasserts dominance.
If confirmed in larger cohorts, the week-3 R/G ratio could serve as a simple prognostic
clinical marker: patients whose R/G ratio remains above approximately $-2$ at the first
follow-up appointment are at elevated short-term relapse risk ($<$4 weeks), whereas
$\text{R/G}_{w3} < -5$ may predict durable remission of at least 6 weeks.

\paragraph{External validation via peri-implant reference distribution.}
A potential concern with the relapse projections is that they extrapolate from
3-week calibration data to 150-day equilibria, with no held-out longitudinal validation.
To partially address this, we compared the model-predicted long-time R/G ratio distribution
to the independent peri-implant dataset of Szafra\'{n}ski et~al.~\citep{szafranski2025}.
Across all 10 Dieckow patients, the mean equilibrium R/G ratio predicted by the model is
$+0.65$ (SD $=0.87$); for 9 of 10 patients the equilibrium R/G ratio falls in the
peri-implantitis (PI) range of Szafra\'{n}ski et~al.\ ($\mu_\mathrm{PI}=+0.53$, $\sigma=1.30$).
By contrast, the mean week-3 R/G ratio for the same patients is $-0.50$, consistent with
the Joshi mucositis/health range ($\mu_\mathrm{Muc}=-1.74$, $\mu_\mathrm{Health}=-2.09$).

The $0.12$-unit offset between our predicted equilibrium ($+0.65$) and the
Joshi PI mean ($+0.53$) falls well within the Joshi SD ($1.30$), and is partly
attributable to the 5-guild zero-imputation required by the incomplete guild
overlap (see Section~\ref{ssec:limitations}).
This concordance provides cross-sectional support for the long-term model extrapolation:
an untreated caries microbiome converging to a R/G ratio indistinguishable from
peri-implantitis is ecologically plausible, as both conditions are characterised by
a shift toward Bacteroidia/Fusobacteria dominance at the expense of Actinobacteria/Streptococcus.
...
\begin{enumerate}
  ...
  \item \textbf{Prior mismatch.} The L1 prior is derived from eHOMD-curated
        ...
        (Duran-Pinedo et~al.~2021, PRJNA725874), the competition weight $\alpha$ shows
  ...
  \item \textbf{Joshi genus coverage.} Only 5 of 10 guilds are directly measured
        (Actinomyces, Streptococcus, Porphyromonas, Fusobacterium, Veillonella); the
        remaining 5 are zero-imputed.  Cross-sectional samples are unlikely to be at
        ODE equilibria, which is why the raw compositional log-ratio outperforms
        ODE-equilibrium R/G ratio ($\rho=0.46$ vs.\ $0.37$--$0.39$).  Adding major
        pathogens (Treponema, Tannerella, Prevotella) would substantially improve
        sensitivity to early dysbiosis stages (Health vs.\ Mucositis).
\end{enumerate}
...
We have shown that a metabolically informed three-layer sign constraint substantially
regularises both Hamilton (symmetric) and gLV (asymmetric) replicator dynamics
for oral microbiome prediction. Key results are:
\begin{itemize}[noitemsep]
  \item \textbf{Best Hamilton}: Hamilton $+$MICOM ($\tau{=}0.5$) achieves
        LOO-RMSE~$=0.0502\pm0.021$, $-$15.6\% vs.\ Hamilton no-prior (0.0595),
        with perfect sign agreement (100\%, 72/72 constrained pairs).
  \item \textbf{Best overall}: gLV $\alpha{=}0.25$ (L1+L2, asymmetric $A$) achieves
        LOO-RMSE~$=0.0490\pm0.016$, $-$16.7\% vs.\ unconstrained gLV (0.0588).
  \item \textbf{NSP comparison}: Hamilton NSP IFT (best variant) achieves
        LOO-RMSE~$=0.091$, $1.9\times$ worse than gLV, due to indirect observables
        and latent-variable warm-up cost.
  \item \textbf{MDSINE2 baseline}: Bayesian gLV (MDSINE2) collapses to A$\approx 0$
        with 3 timepoints per patient; RMSE~$=0.055$ ($+12\%$ vs.\ gLV $\alpha{=}0.25$).
        Sign-prior regularisation is superior to Bayesian sparsity for small $T$.
  \item LOO Bray-Curtis $=0.147$ ($-$5.3\% vs.\ L1+L2, $-$47.7\% vs.\ persistence null).
  ...
  \item Robust sign stability (SC$=1.00$ for 31/45 pairs; 40/45 at SC$\geq 0.90$) across LOO folds.
  \item External validity: Dieckow $\bA$ correctly orders peri-implant dysbiosis
        in 127 independent cross-sectional samples (Spearman $\rho=0.46$, $p<10^{-7}$;
        binary Health vs.\ PI $\rho=0.59$, $p<10^{-8}$).
\end{itemize}
Key limitations are the small cohort ($N=10$, $p=0.08$ for the AGORA improvement),
the symmetry constraint of the Hamilton model, and the caries-context prior.
Future work should replicate in larger cohorts, evaluate asymmetric gLV with
asymmetric MICOM sign priors, construct disease-specific priors for periodontitis,
incorporate posterior uncertainty via TMCMC~\citep{nishioka_paper}, and validate
...
\section{\texorpdfstring{$\sigma$}{sigma} sensitivity of constraint penalty}
\label{app:sigma}
...
\section{gLV reaction-diffusion PDE: depth profiles across LOO folds}
\label{app:pde}

\begin{figure}[htb]
\centering
\includegraphics[width=0.90\linewidth]{fig_pde_gradient_boxplot.pdf}
\caption{Substratum enrichment index $\Delta\phi_i = \phi_i(z{=}0) - \phi_i(z{=}1)$
         at $t=21$\,d for the gLV 1D reaction-diffusion PDE across all 10 LOO folds
         (placeholder diffusivities $D_i$, advection $u=0.005$, $N_z=30$).
         Positive values indicate enrichment at the substratum ($z=0$, enamel surface);
         negative values indicate enrichment in the bulk ($z=1$).
         Actinobacteria and Bacilli are consistently enriched at the substratum
         (mean $\Delta\phi = +0.115$ and $+0.078$, respectively), consistent with
         their role as early colonisers. Negativicutes and $\gamma$-Proteobacteria
         are consistently depleted ($-0.050$ and $-0.037$). $\beta$-Proteobacteria
         show high fold-to-fold variance ($\pm 0.200$), reflecting sensitivity to
         the held-out patient's $A$ matrix. Note: $D_i$ are placeholder values
         derived from literature ranges; quantitative conclusions require $z$-resolved
         CLSM depth profiles (pending Szafra\'{n}ski lab data request).}
\label{fig:pde_gradient}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=\linewidth]{fig_pde_depth_profiles_all_folds.pdf}
\caption{gLV PDE guild depth profiles $\phi_i(z)$ at $t=21$\,d for all 10 LOO
         folds (thin lines) and fold mean (bold). One panel per guild, $z=0$ is
         substratum, $z=1$ is bulk. Guilds with consistent fold-to-fold behaviour
         (Actin., Negat.) show tight bands; $\beta$-Prot. shows large inter-fold
         spread.}
\label{fig:pde_profiles}
\end{figure}

% ─── Data and Code Availability ───────────────────────────────────────────────
\section*{Data and Code Availability}
The analysis code, fitted model parameters, and LOO-CV results are available at
\url{https://github.com/keisuke58/nife\_intern}.
The 16S rRNA amplicon time-series data are from Dieckow et al.~\citeyear{dieckow2024}
(available from the corresponding author on reasonable request).
The peri-implant cross-sectional 16S data (Joshi et al.) are deposited in NCBI SRA
under accession PRJNA1192962.
...
Joshi AA, Szafrański SP, Steglich M, et~al. (2025). Integrative microbiome- and
metatranscriptome-based analyses reveal diagnostic biomarkers for peri-implantitis.
\textit{npj Biofilms and Microbiomes} 11:175. doi:10.1038/s41522-025-00807-6.
...
Klempt F, Soleimani M, Wriggers P, Junker P (2024).
A Hamilton principle-based model for diffusion-driven biofilm growth.
\textit{Biomechanics and Modeling in Mechanobiology} 23:2091--2113. doi:10.1007/s10237-024-01883-x.

\bibitem[Klempt et~al.(2025)]{klempt2025}
Klempt F, Geisler H, Soleimani M, Junker P (2025). A continuum multi-species
biofilm model with a novel interaction scheme. \textit{arXiv:2509.01274 [q-bio.QM]}.
