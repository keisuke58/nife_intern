...
\begin{equation}
  ...
  \label{eq:gdi}
\end{equation}
without ODE integration.  We compared this \emph{compositional} GDI against
model-based variants (Hamilton ODE equilibrium, instant GDI $= -\bphi^\top(\bb+\bA\bphi)$
...
diagnostic groups in the Joshi dataset.  Health and Mucositis have similar GDI
...
$\rho(\text{diagnosis severity}, \text{GDI}) = 0.46$, $p=5\times10^{-8}$
...
\caption{Guild Dysbiosis Index by diagnosis (Joshi et~al.\ 2025, 127 samples).
         GDI $=\log(\phi_\mathrm{dys})-\log(\phi_\mathrm{com})$ evaluated at
...
\caption{External validation: Guild Dysbiosis Index in Joshi~et~al.~(2025) peri-implant
         ...
         (GDI $>0$).  \textbf{(B)} Comparison of GDI variants: the raw compositional
...
Dysbiosis Index (GDI~$= \log\phi_{\text{dys}} - \log\phi_{\text{com}}$).
\emph{No value of $\alpha$ drives the mean GDI below zero} (tipping point
...
To identify which individual growth rates most strongly shift GDI, we swept
each $b_i$ from its CT2 to CT1 value while holding all others at CT2 levels.
The two largest GDI drivers are Gammaproteobacteria
($|\Delta\text{GDI}|=1.16$; $\Delta b = -5.31$, i.e.\ CT1 patients have
substantially lower Gammaproteobacteria growth rate) and Bacteroidia
($|\Delta\text{GDI}|=0.93$; $\Delta b = -0.96$).
...
the community toward a commensal GDI — despite Gammaproteobacteria having
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Transient responders (3/10 patients: C, G, L):}
        GDI $<0$ at week 3 (apparent commensal recovery), but the model predicts
        a GDI crossing to positive at days 24, 38, and 48, respectively.
        Patient L shows the most striking profile: GDI~$=-5.66$ at week 3
  ...
  \item \textbf{Permanent commensal (1/10 patients: D):}
        GDI remains $<0$ throughout, converging to a stable commensal equilibrium.
...
community (transient GDI reduction) but does not permanently alter the
...
\caption{Predicted relapse dynamics: GDI trajectories for all 10 patients
         simulated to 90 days using patient-specific gLV
         ($\bA$, $\hat{\bb}^{(p)}$, week-0 $\phi_0^{(p)}$).
         Green shading: 3-week data window. Dashed line: GDI~$=0$ boundary
...
For patients C, G and L — who each showed commensal-like GDI at week 3 but subsequently
...
identified the minimum reduction that eliminates the later GDI sign-change (i.e.\ achieves
...
\paragraph{Week-3 GDI as a prognostic marker.}
Among the three transient responders, the week-3 GDI correlates inversely with relapse day:
patient~L achieves the most commensal-like state at week 3
($\mathrm{GDI}_{w3} = -5.66$, relapse: day 48), patient~C
($\mathrm{GDI}_{w3} = -2.15$, relapse: day 24), and patient~G
($\mathrm{GDI}_{w3} = -1.20$, relapse: day 38).
While $N=3$ precludes statistical inference, this ordering is consistent with the hypothesis
that a more negative week-3 GDI reflects deeper penetration into the commensal basin,
buying additional time before the dysbiotic attractor reasserts dominance.
If confirmed in larger cohorts, the week-3 GDI could serve as a simple prognostic
clinical marker: patients whose GDI remains above approximately $-2$ at the first
follow-up appointment are at elevated short-term relapse risk ($<$4 weeks), whereas
$\mathrm{GDI}_{w3} < -5$ may predict durable remission of at least 6 weeks.
...
To partially address this, we compared the model-predicted long-time GDI distribution
to the independent peri-implant dataset of Joshi et~al.~\citep{joshi2025}.
Across all 10 Dieckow patients, the mean equilibrium GDI predicted by the model is
$+0.65$ (SD $=0.87$); for 9 of 10 patients the equilibrium GDI falls in the
peri-implantitis (PI) range of Joshi et~al.\ ($\mu_\mathrm{PI}=+0.53$, $\sigma=1.30$).
By contrast, the mean week-3 GDI for the same patients is $-0.50$, consistent with
...
an untreated caries microbiome converging to a GDI indistinguishable from
...
\begin{enumerate}
  ...
  \item \textbf{Joshi genus coverage.} Only 5 of 10 guilds are directly measured
        ...
        ODE-equilibrium GDI ($\rho=0.46$ vs.\ $0.37$--$0.39$).  Adding major
