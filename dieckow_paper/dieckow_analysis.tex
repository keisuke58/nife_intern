...
To validate the stability of these centrality rankings, we performed
leave-one-patient-out (LOO) re-fitting of $\bA$ on 9/10 patients for each fold.
61 of 90 off-diagonal pairs (68\%) achieve sign consistency SC~$\geq 0.9$
across all 10 folds; the 10 pairs with SC$=1.0$ and largest magnitudes
are all centred on Actinobacteria and Bacilli interactions
($A_{\text{Baci,Beta}}=-2.10$, $A_{\text{Acti,Beta}}=+1.74$,
$A_{\text{Acti,Bact}}=+0.90$, etc.),
confirming that the centrality conclusions are robust to individual patient removal.
