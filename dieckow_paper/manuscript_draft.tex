...
We introduce a three-layer sign prior that combines experimental interaction
...
An AGORA weight of $W{=}1.0$ achieves a phase transition to 100\% sign
...
pairs symmetrically;
(ii)~literature-curated sign constraints from co-culture experiments, which
are sparse and biased toward well-studied pairs; and
...
Here we propose a three-layer sign prior that unifies experimental evidence
...
\begin{enumerate}[noitemsep]
  \item A three-layer sign prior integrating AGORA2 FBA into gLV inference,
  ...
  \item Demonstration that $W{=}1.0$ is a biologically motivated calibration
        point (L2-equivalent weight), exhibiting a phase transition in sign
        agreement.
...
Inter-guild interactions are constrained by a three-layer sign prior that
...
sign prior: positive $F^\text{sym}_{ij}$ predicts positive $A_{ij}$.
Weight $W \in \{0.5, 1.0, 1.5, 2.0\}$ is swept; $W{=}1.0$ is adopted
(§\ref{ssec:wSweep}).

The combined sign penalty is:
\begin{equation}
...
of the sign prior from that of the symmetry assumption.
...
\caption{AGORA2 genome-scale metabolic prior construction.
         ...
         $F_{ij}$ (secretion $-$ uptake), and sign prior $\sgn(F_{ij}) \to$
         penalty.
...
The 6 discordant pairs all involve the ``Other'' guild, where AGORA2 coverage
is absent by construction; restricting to the 58 pairs with at least one
guild having a curated AGORA2 representative raises agreement to 96.6\%.

The MacArthur quantitative prior~\citep{marsland2019} fails entirely:
sign agreement drops to 4--11/70 (6--16\%) across the OLS-calibrated
concentration parameters tested ($c \in \{0.01, 0.05, 0.10, 0.50\}$,
Table~\ref{tab:comparison}), confirming that FBA flux magnitudes do not
transfer to gLV interaction units.
Sign information (direction of cross-feeding), not magnitude (flux amount),
is the ecologically predictive quantity from FBA.

\begin{figure}[htb]
\centering
\includegraphics[width=0.88\linewidth]{fig3_agora_sign_validation}
\caption{AGORA2 sign prior validation.
...
\subsection{W\,=\,1.0 Phase Transition in Sign Agreement}
\label{ssec:wSweep}
...
then returns to 94\% at $W{=}1.5$, tracing a discrete phase transition.
...
\caption{AGORA weight sweep ($W \in \{0, 0.5, 1.0, 1.5, 2.0\}$).
         \textbf{(A)} Sign agreement: discrete phase transition at $W{=}1.0$
         ...
         coincides with SA phase transition.}
\label{fig:wsweep}
\end{figure}

\subsection{LOO-CV: Sign Prior Improves Generalisation}
...
LOO improves for 8/10 patients with the sign prior relative to
...
falls in this regime: despite strong metabolic evidence the interaction
is not ecologically dominant on the 3-week abutment window,
consistent with the hypothesis that competition for space or nutrients
outweighs metabolic syntrophy at this timescale~\citep{mikx1975}.
...
Three non-exclusive explanations are consistent with this result:
(i) the 3-week abutment window is too short to resolve the ecological impact
of a metabolic interaction operating on longer timescales;
(ii) competition for shared carbon sources dominates over the lactate
syntrophy at the relative-abundance level; and
(iii) guild-level aggregation merges distinct Streptococcus and Veillonella
phylotypes with different metabolic capacities.
The prediction that longer time-series would detect a stronger
Bacilli--Negativicutes $A$ is testable with the peri-implantitis cohort
of Szafra\'{n}ski~et~al.
...
A weight of $W{=}1.0$ (L2-equivalent) achieves a phase transition to
